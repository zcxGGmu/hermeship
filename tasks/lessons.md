...
## 2026-06-27: 官网首屏项目标识只能有一个主视觉

- 用户纠正：产品官网开头位置不要堆叠导航图标、居中图标、lockup banner 和重复标题。
- 后续调整官网 hero 时，首屏主体默认只保留一个居中的项目图标作为主视觉。
- 如果需要保留语义标题，应使用无视觉干扰的隐藏 `h1` 或由正文承接，不再叠加可见品牌图和重复标题。
- 导航栏可以保留文字品牌，但不要在首页首屏同时展示第二个项目图标，避免品牌识别噪音。
